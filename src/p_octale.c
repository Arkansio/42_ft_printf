...
/*   Updated: 2019/01/19 04:30:54 by mgessa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		print(t_proper *properties, int sz_result,
long long val, char *str)
{
	...
	if (contain_flag(properties, diez) && ((properties->precision
	<= sz_result && val != 0) || (properties->precision == 0 && val == 0)))
		ft_putchar('0');
	if (val != 0 || properties->precision != 0)
		ft_putfaststr(str, -1);
	if (contain_flag(properties, minus))
...
int				p_octale(t_proper *properties, va_list *args)
{
	...
	if (properties->precision == 0 && val == 0)
		sz_result--;
	if (contain_flag(properties, diez) && ((properties->precision <= sz_result
	&& val != 0) || (properties->precision == 0 && val == 0)))
		sz_result++;
	print(properties, sz_result, val, str);
